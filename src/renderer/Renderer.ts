...
import { cubeShader } from './cubeShader';

const DEPTH_FORMAT: GPUTextureFormat = 'depth24plus';

/**
 * Owns the render pipeline, frame loop, and render-target-sized resources.
 * Scene geometry remains in Cube so additional objects can be introduced without
...
export class Renderer {
  ...
  constructor(
    ...
    this.cube = new Cube(this.device);
    ...
    const shaderModule = this.device.createShaderModule({ code: cubeShader });
